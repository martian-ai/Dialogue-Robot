"""
This module implements the short text similarity based on:
1. the ESIM algorithm described in https://arxiv.org/pdf/1609.06038.pdf
2. (Drop) the ESIM code described in https://github.com/pengshuang/Text-Similarity/blob/master/models/ESIM.py
3. the ESIM code described in https://github.com/coetaur0/ESIM/blob/master/esim/model.py
4. Code inspired from: https://github.com/allenai/allennlp/blob/master/allennlp/nn/util.py.
"""
import torch
...
class ESIM(nn.Module):
    ...
    def __init__(self,
        ...
        self._word_embedding.weight.data.copy_(torch.from_numpy(embeddings))
...
if __name__ == "__main__":
    ...
    from solutions.utils.dataset.Dataset_Retrieval import Dataset_ChineseSTS
    ...
    model = ESIM(len(tokenizer.word2idx), len(tokenizer.embedding_matrix[0]), 200, np.asarray(tokenizer.embedding_matrix))

    from torch.utils.data import DataLoader
    data_loader = DataLoader(dataset=instances, batch_size=8, shuffle=True)
    print(data_loader.batch_size)

    for t_batch, t_sample_batched in enumerate(data_loader):
        t_targets = t_sample_batched['score']
        tmp_shape = np.asarray([16,16,16,16,16,16,16,16])
        t_outputs = model(t_sample_batched['a'], tmp_shape, t_sample_batched['b'], tmp_shape)
...
class SEQ2SEQNet:  # 创建一个SEQ2SEQNet类
    ...
    def forward(self):
        ...
        with tf.variable_scope('encode'):  # 创建一个变量空间 encode
            ...
            self.encoder_states = self.encoder_cell.zero_state(100, dtype=tf.float32)  # 初始化细胞的状态为 0, 传入初始化批次 和数据类型, 指定batch_size(当前为100), shape 是 batch_size * num_units
            ...
            # time_major 决定 tensor的格式
            # 如果是True，tensor 形状必须为[max_time, batch_size, cell.output_size]
            # 如果是False， tensor的形状为[batch_size, max_time, cell.output_size] 

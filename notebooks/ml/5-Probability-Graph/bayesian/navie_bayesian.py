...
def getAllProbability(trainDataArr, trainLabelArr):
    ...
    for i in range(classNum):
        ...
        Py[i] = ((np.sum(np.mat(trainLabelArr) == i)) + 1) / (len(trainLabelArr) + 10) # 所有为1的个数 / 总个数 
    #转换为log对数形式
...
if __name__ == "__main__":
    ...
    trainDataArr, trainLabelArr = loadData('../corpus/mnist_train.csv')

    # 获取测试集
    print('start read testSet')
    testDataArr, testLabelArr = loadData('../corpus/mnist_test.csv')

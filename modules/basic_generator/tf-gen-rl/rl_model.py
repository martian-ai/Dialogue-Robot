...
class Model(nn.Module):
    def __init__(self, args, device, vocab, checkpoint=None):
        ...
        if args.encoder == 'bert':
            ...
            self.encoder = TransformerEncoder(self.hidden_size, args.enc_ff_size, args.enc_heads, args.enc_dropout, args.enc_layers)
            tgt_embeddings = nn.Embedding(self.vocab_size, self.hidden_size, padding_idx=0)

        self.sent_encoder = TransformerEncoder(self.hidden_size, args.enc_ff_size, args.enc_heads, args.enc_dropout, args.enc_layers)
        self.hier_encoder = TransformerEncoder(self.hidden_size, args.hier_ff_size, args.hier_heads, args.hier_dropout, args.hier_layers)
        ...
        for idx in range(len(ex_segs)):
            ...
            for j, sent in enumerate(src_cand):
                ...
                if sep_token is not None:
                    filted_sent = torch.cat([filted_sent, torch.tensor([sep_token], device=self.device)], -1)
    ...
    def _base_summary_generate(self, batch, topic_info):
        ...
        pn_result = self._pointer_net_decoding(batch, hier, self.pn_init_token, self.args.max_pos, memory_mask=sent_mask, method="max", topic_vec=topic_vec_pn)
        ext_labels = [t[0][0].tolist() for t in pn_result]
        src_mapped, segs_mapped, mask_src_mapped, agent_mask, customer_mask, idx_mapped = self._map_src(src, ext_labels, ex_segs, self.seg_token)
    ...
    def _pointer_net_decoding(self, batch, memory_bank, init_tokens, max_length, memory_mask=None, min_length=2, topic_vec=None, method="sample"):

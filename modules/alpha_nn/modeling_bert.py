...
from modules.alpha_nn.function.file_utils import cached_path
from modules.alpha_nn.function.loss import LabelSmoothingLoss
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, history_states=None, mask_qkv=None, seg_ids=None):
        self_output = self.self(input_tensor, attention_mask, history_states=history_states, mask_qkv=mask_qkv, seg_ids=seg_ids)
...
class PreTrainedBertModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name, state_dict=None, cache_dir=None, *inputs, **kwargs):
        ...
        for qkv_name in ('query', 'key', 'value'):
            _k = 'bert.encoder.layer.0.attention.self.{0}.weight'.format(qkv_name)
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True, mask_qkv=None, task_idx=None):
        ...
        encoded_layers = self.encoder(embedding_output, extended_attention_mask, output_all_encoded_layers=output_all_encoded_layers, mask_qkv=mask_qkv, seg_ids=token_type_ids)
...
class BertForPreTraining(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None, next_sentence_label=None, mask_qkv=None, task_idx=None):
        sequence_output, pooled_output = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False, mask_qkv=mask_qkv, task_idx=task_idx)
...
class BertForPreTrainingLossMask(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2, num_rel=0, num_sentlvl_labels=0, no_nsp=False):
        ...
        self.bert = BertModel(config)
        self.cls = BertPreTrainingHeads(config, self.bert.embeddings.word_embeddings.weight, num_labels=num_labels)
        self.num_sentlvl_labels = num_sentlvl_labels
        self.cls2 = None
        if self.num_sentlvl_labels > 0:
            self.secondary_pred_proj = nn.Embedding(num_sentlvl_labels, config.hidden_size)
            self.cls2 = BertPreTrainingHeads(config, self.secondary_pred_proj.weight, num_labels=num_sentlvl_labels)
        ...
        if self.num_rel > 0:
            self.crit_pair_rel = BertPreTrainingPairRel(config, num_rel=num_rel)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
        ...
        if token_type_ids is None and attention_mask is None:
            ...
            index_matrix = torch.arange(sequence_length).view(1, sequence_length).to(input_ids.device)
            ...
            attention_mask_task_0 = (index_matrix < num_tokens.view(-1, 1, 1)) & (index_matrix_t < num_tokens.view(-1, 1, 1))
        ...
        if self.cls2 is not None and masked_pos_2 is not None:
            sequence_output_masked_2 = gather_seq_out_by_pos(sequence_output, masked_pos_2)
            prediction_scores_masked_2, _ = self.cls2(sequence_output_masked_2, None)
            masked_lm_loss_2 = self.crit_mask_lm(prediction_scores_masked_2.transpose(1, 2).float(), masked_labels_2)
            masked_lm_loss_2 = loss_mask_and_normalize(masked_lm_loss_2.float(), masked_weights_2)
        ...
        if pair_x_mask is None or pair_y_mask is None:
            ...
            pair_x_output_masked = gather_seq_out_by_pos_average(sequence_output, pair_x, pair_x_mask)
            pair_y_output_masked = gather_seq_out_by_pos_average(sequence_output, pair_y, pair_y_mask)
        pair_loss = self.crit_pair_rel(pair_x_output_masked, pair_y_output_masked, pair_r, pair_pos_neg_mask)
...
class BertForExtractiveSummarization(PreTrainedBertModel):
    ...
    def __init__(self, config):
        ...
        self.cls2 = BertPreTrainingHeads(config, self.secondary_pred_proj.weight, num_labels=2)
        self.apply(self.init_bert_weights)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_pos_2=None, masked_weights_2=None, task_idx=None, mask_qkv=None):
        ...
        sequence_output_masked_2 = gather_seq_out_by_pos(sequence_output, masked_pos_2)
        prediction_scores_masked_2, _ = self.cls2(sequence_output_masked_2, None, task_idx=task_idx)

        predicted_probs = torch.nn.functional.softmax(prediction_scores_masked_2, dim=-1)
...
class BertForSeq2SeqDecoder(PreTrainedBertModel):
    ...
    def __init__(self, config, mask_word_id=0, num_labels=2, num_rel=0, search_beam_size=1, length_penalty=1.0, eos_id=0, sos_id=0, forbid_duplicate_ngrams=False, forbid_ignore_set=None, not_predict_set=None, ngram_size=3, min_len=0, mode="s2s", pos_shift=False):
        ...
        while next_pos < output_length:
            ...
            if self.pos_shift:  # TODO
                ...
                if prev_encoded_layers is None:
                    ...
                    prev_encoded_layers = [torch.cat((x[0], x[1]), dim=1) for x in zip(prev_encoded_layers, new_encoded_layers)]
            else:
                if prev_embedding is None:
...
class BertForSequenceClassification(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, mask_qkv=None, task_idx=None):
        _, pooled_output = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False, mask_qkv=mask_qkv, task_idx=task_idx)
...
class BERT_CLF(PreTrainedBertModel):
    def __init__(self, config, opt):
        """
        config : bert 参数
        opt : 分类模型等参数
        """
        super(BERT_CLF, self).__init__(config)
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(opt.dropout)
        self.dense = nn.Linear(opt.bert_dim, opt.clf_dim)

    def forward(self, indice, mask, segment):
        # indices = inputs[0].squeeze(1)
        # context, attention_mask=mask, output_all_encoded_layers=False
        _, pooled_output = self.bert(input_ids = indice , attention_mask=mask, token_type_ids=segment)  # token_type_ids 用来区分是第一句还是第二句
        # _, pooled_output = self.bert(indices, return_dict=False) # token_type_ids 用来区分是第一句还是第二句
        # https://stackoverflow.com/questions/65082243/dropout-argument-input-position-1-must-be-tensor-not-str-when-using-bert
        result = nn.functional.softmax(self.dense(pooled_output))
        return result
...
class BERT_CRF(PreTrainedBertModel):
    def __init__(self, config, opt):
        ...
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(opt.dropout)
        self.dense = nn.Linear(opt.bert_dim, opt.label_dim)
    ...
    def forward(self, inputs):
        indices = inputs[0].squeeze(1)
        _, pooled_output = self.bert(indices)
        dense_output = self.dense(self.dropout(pooled_output))
        score, tag_seq = self._viterbi_decode(dense_output)

...
from utils.trie import load_tree

ANY_SPACE = '<SPACE>'

def evaluate(sess, model, data, id_to_tag, input_dir, output_dir, file_name):
    dict_file = os.path.join(input_dir, "config_files", "dict.txt")
 
    words_tree, words_type = None, []
    if os.path.exists(dict_file):
        words_tree, words_type = load_tree(dict_file)

    if words_tree is None:
        print("无词表测试")
        ner_results = model.evaluate(sess, data, id_to_tag)
    else:
        print("有词表测试")
        ner_results = model.evaluate(sess, data, id_to_tag, words_tree, words_type)
    eval_lines = test_ner(ner_results, output_dir, file_name)
    ...
    f1 = float(eval_lines[1].strip().split()[7])

    return precision, recall, f1

def return_report(input_file):
    print('input_file return', input_file)
    with codecs.open(input_file, "r", "utf8") as f:
        counts = p_r_f1_for_flie(f)
    return report_notprint(counts)

def test_ner(results, path, file_name):
    """
...
def p_r_f1_for_flie(iterable, options=None):
    if options is None:
...
def main(argv):
    ...
    if args.file is None:
        counts = p_r_f1_for_flie(sys.stdin, args)
    else:
        with open(args.file) as f:
            counts = p_r_f1_for_flie(f, args)
    report(counts)

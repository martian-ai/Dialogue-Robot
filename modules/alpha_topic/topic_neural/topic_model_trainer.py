...
from loss import abs_loss, LogisticLossCompute
...
def build_trainer(args, device_id, model, optims, tokenizer):
    ...
    trainer = Trainer(args, model, optims, tokenizer, grad_accum_count, n_gpu, gpu_rank, report_manager)
...
class Trainer(object):
    ...
    def __init__(self,  args, model,  optims, tokenizer,
            ...
            for i, batch in enumerate(train_iter):
                if self.n_gpu == 0 or (i % self.n_gpu == self.gpu_rank):
                    ...
                    if accum == self.grad_accum_count:
                        ...
                        self._gradient_calculation( true_batchs, examples, total_stats, report_stats, step)
                        report_stats = self._maybe_report_training( step, train_steps, self.optims[0].learning_rate, report_stats)
                        true_batchs = []
                        accum = 0
                        examples = 0
    ...
    def _gradient_calculation(self, true_batchs, examples, total_stats, report_stats, step):
        ...
        for batch in true_batchs:
            ...
            # Topic Model loss
            topic_stats = Statistics(topic_loss=loss.clone().item() / float(examples))
        ...
        if step % 1000 == 0:
            for k in range(self.args.topic_num):
                logger.info(','.join([self.model.voc_id_wrapper.i2w(i) for i in self.model.topic_model.tm1.beta.topk(20, dim=-1)[1][k].tolist()]))

...
class WordTopicMatrix:
    ...
    def get_new_topic(self):
        ...
        new_topic = 0
        while True:
            if new_topic not in self.topicSum:
                return new_topic
            new_topic += 1
    ...
    def get_conditional(self, wordNo, topicNo, eta):
        ...
        """
        if topicNo not in self.topicSum:
...
class Document:
    ...
    def get_new_table(self):
        ...
        new_table = 0
        while True:
            if new_table not in self.tableSum:
                break
            new_table += 1
        return new_table
    ...
    def add_word_to_table(self, wordNo, tableNo):
        ...
        """
        if tableNo not in self.tableSum:
            self.tableWords[tableNo] = dict()
            self.tableSum[tableNo] = 0

        self.tableSum[tableNo] += 1

        # if not self.tableWords[tableNo].has_key():
        if wordNo not in self.tableWords: 
        ...
        for doc in self.doc_list:
            ...
            for word in doc.word_list:
                ...
                for topicNo in topic_prob.keys():
                    if topicNo in self.table_assigned_topics:

...
def get_corpus_ids_cnt(corpus_path='corpus.txt', vocab_path='vocab.txt', stopwords_path='stop.txt', num_doc=100, max_voca=10000, remove_top_n=5):
    ...
    for item in lines[:num_doc]:
        corpus.append([ tmp for tmp in list(jieba.cut(item)) if tmp not in stopwords])

...
from solutions.qa.answer_interceptor.bert_qa import evaluate
from solutions.qa.answer_interceptor.utils_squad import (read_squad_examples, convert_examples_to_features, RawResult, write_predictions, write_predictions_extended)
from solutions.qa.answer_interceptor.utils_squad import SquadExample
from solutions.search.recall.elastic_search import es_search
...
def get_search_results(query, search_size, search_engine='offline-es'):
    if search_engine == 'offline-es':
        results_search = es_search(query, search_size, 'three-body')
...
def predict(args, query, model, tokenizer, prefix=""):
    ...
    for batch in eval_dataloader:
        ...
        with torch.no_grad():
            inputs = {'input_ids': batch[0], 'attention_mask': batch[1], 'token_type_ids': None if args.model_type == 'xlm' else batch[2] } # XLM don't use segment_ids
